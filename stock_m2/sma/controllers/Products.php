...
class Products extends MY_Controller
{
        ...
        if (!empty($options)) {
            ...
            foreach ($options as $option) {
                ...
                for($i=0; $i < $quantity; $i++) {
                    ...
                    $table .= '<td style="width: 20px;"><table class="table-barcode"><tbody><tr><td colspan="2" class="bold">' . $this->Settings->site_name . '</td></tr><tr><td colspan="2">' . $product->name . ' - '.$option->name.'</td></tr><tr><td colspan="2" class="text-center bc">' . $this->product_barcode($product->code . '~' . $option->id, 'code128', 60) . '</td></tr>';
        ...
        if (!empty($options)) {
            ...
            foreach ($options as $option) {
                ...
                for($i=0; $i < $quantity; $i++) {
                    ...
                    $table .= '<td style="width: 20px;"><table class="table-barcode"><tbody><tr><td colspan="2" class="bold">' . $this->Settings->site_name . '</td></tr><tr><td colspan="2">' . $product->name . ' - '.$option->name.'</td></tr><tr><td colspan="2" class="text-center bc">' . $this->product_barcode($product->code . '~' . $option->id, 'code128', 30) . '</td></tr>';
                    foreach ($currencies as $currency) {
                        $table .= '<tr><td class="text-left">' . $currency->code . '</td><td class="text-right">' . $this->sma->formatMoney($product->price * $currency->rate) . '</td></tr>';
                    }
                    $table .= '</tbody></table>';
        ...
        if (!empty($options)) {
            foreach ($options as $option) {
                for ($r = 1; $r <= $option->wh_qty; $r++) {
                    $html .= '<div class="labels"><strong>' . $pr->name . ' - '.$option->name.'</strong><br>' . $this->product_barcode($pr->code . '~' . $option->id, 'code128', 25) . '<br><span class="price">'.lang('price') .': ' .$this->Settings->default_currency. ' ' . $this->sma->formatMoney($pr->price) . '</span></div>';
                }
            }
        } else {
    ...
    function print_barcodes($product_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $bci_size = ($style == 10 ? 50 : ($style == 14 ? 30 : 20));
            $currencies = $this->site->getAllCurrencies();
                    ...
                    foreach ($variants as $option) {
                        if ($this->input->post('vt_'.$product->id.'_'.$option->id)) {
                            $barcodes[] = array(
                                ...
                                'barcode' => $this->product_barcode($product->code . '~' . $option->id, 'code128', $bci_size),
