<?php
//*WorK on a better way to make an order as a superadmin currently its imposible*//
if (!defined('BASEPATH')) {
...
//echo $this->session->userdata('country_tax'); exit;
        $data['product'] = $this->order_model->get_all_product_info();
        $data['countries'] = $this->global_model->get_countries();
    ...
    public function add_cart_item(){
            ...
            if($result){
                ...
                //product tax check
                //$result->tax_id -> County's tax
                $tax = $this->product_tax_calculate($this->session->userdata('country_tax'), $qty=1, $price);
        ...
        if($qty !=0 )
        {
            ...
            $product_inventory = $this->global_model->get_by(array('product_id'=> $result->product_id ), true);
               ...
               //Create a Variable from a string
                $field = '$'.'product_inventory->product_quantity_'.country($this->session->userdata('user_country'));
                eval("\$field = \"$field\";");
            if($qty > $field)
            {
                $type = 'error';
            ...
            //product tax check
            $tax = $this->product_tax_calculate($this->session->userdata('country_tax'), $qty, $price);
    ...
    public function save_order()
    {
       ...
       $this->global_model->log('added an order:'.$order_code);
        ...
        if(empty($customer_code))
        {
            ...
            $data_order['country_id'] = $this->session->userdata('user_country');
            $data_order['customer_email']= '';
            $data_order['customer_phone']= '';
            $data_order['customer_address']= '';
            $data_order['shipping_address']= '';
        ...
        }else
        {
            ...
            $data_order['country_id'] = $customer_info->country_id;
...
//echo json_encode($data_order); exit;
        ...
        foreach($cart as $item)
        {
            ...
            $inchi = country($customer_info->country_id);
            //Create a Variable from a string
                $field = '$'.'inventory->product_quantity_'.country($customer_info->country_id);
                eval("\$field = \"$field\";");

            $inventory_qty['product_quantity_'.$inchi] = $field - $item['qty'];
...
//echo json_encode($data); exit;
    ...
    public function pdf_invoice($id=null)
    {
        ...
        $this->tbl_order('order_id');
        $data['order_info']= $this->global_model->get_by(array('order_id'=>$data['invoice_info']->order_id), true);
        $this->global_model->log('generated an invoice for order:#'.$data['invoice_info']->order_id);
       ...
       // echo json_encode($data); exit;
        ...
        $filename = 'INV-'.$id;
        $this->pdf->pdf_create($html, $filename, 'download');
    ...
    public function pdf_receipt($id=null)
    {
        ...
        $this->global_model->log('generated a receipt for order:#'.$data['invoice_info']->order_id);
        
        $html = $this->load->view('admin/order/pdf_order_invoice_receipt', $data, true);
        $filename = 'Receipt-'.$id;
        $this->pdf->pdf_create($html, $filename, 'download');
        ...
        /*** Dispatch PDF  ***/
    public function pdf_dispatch($id=null){
        ...
        $this->tbl_order_details('order_details_id');
        $data['order_details']= $this->global_model->get_by(array('order_id'=>$data['order_info']->order_id), false);
        $this->global_model->log('generated a Dispatch note for order:#'.$data['order_info']->order_id);
       ...
       // echo json_encode($data); exit;
        ...
        $html = $this->load->view('admin/order/pdf_order_dispatch', $data, true);
        $filename = 'Dispatch-'.$id;
        $this->pdf->pdf_create($html, $filename, 'download');
    }

        /*** Quote PDF  ***/
    public function pdf_quote($id=null){
        ...
        $this->tbl_order_details('order_details_id');
        $data['order_details'] = $this->global_model->get_by(array('order_id'=>$data['order_info']->order_id), false);
        $this->global_model->log('generated a quotation for order:#'.$data['order_info']->order_id);
        ...
        //get invoice
        $data['order'] = $this->order_model->get_all_order();
        $data['title'] = 'Order Quote';
       // echo json_encode($data); exit;
        
        $html = $this->load->view('admin/order/pdf_order_quote', $data, true);
        $filename = 'Quote-'.$id;
        $this->pdf->pdf_create($html, $filename, 'download');
    }
    ...
    public function email_invoice($id=null){
        ...
        $this->tbl_order_details('order_details_id');
        $data['order_details']= $this->global_model->get_by(array('order_id'=>$data['invoice_info']->order_id), false);
        $this->global_model->log('emailed an invoice for order:#'.$data['invoice_info']->order_id.' to '.$data['order_info']->customer_email);
        ...
        if(!empty($company_info->email) && !empty($company_info->email)) {
            ...
            $data['doc_name'] = 'Invoice no:' . $id;
            $data['date'] = $data['invoice_info']->invoice_date;

            // set view page

            $body_data = $this->load->view('admin/order/mail/email_template', $data, true);
    ...
    public function email_quote($id=null){
        ...
        $this->tbl_order_details('order_details_id');
        $data['order_details']= $this->global_model->get_by(array('order_id'=>$data['order_info']->order_id), false);
        $this->global_model->log('emailed a quotation for order:#'.$data['order_info']->order_id.' to '.$data['order_info']->customer_email);
        ...
        if(!empty($company_info->email) && !empty($company_info->email)) {
            ...
            $data['date'] = $data['order_info']->order_date;
            // set view page

            $body_data = $this->load->view('admin/order/mail/email_template', $data, true);
            $view_page = $this->load->view('admin/order/pdf_order_quote', $data, true);
            $send_email = $this->mail->sendPdf($body_data, $to, $subject, $view_page);


            if (!isset($send_email)) {
                $this->message->custom_success_msg('admin/order/view_order/' . $id,
                    'Your email has been sent successfully!');
            } else {
                $this->message->custom_error_msg('admin/order/view_order/' . $id,
                    'Sorry we are unable to send your email!');
            }
        }else{
                 $this->message->custom_error_msg('admin/order/view_order/' . $id,
