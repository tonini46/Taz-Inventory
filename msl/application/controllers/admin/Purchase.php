...
class Purchase extends Admin_Controller
{
    ...
    public function save_purchase()
    {
        ...
        $data = $this->global_model->array_from_post(array('supplier_id', 'purchase_ref', 'payment_method', 'payment_ref', 'country_id'));
        ...
        foreach($cart as $item)
        {
            ...
            if(!empty($product)) {
                ...
                $country = country($this->input->post('country_id'));
                $inventory = $this->global_model->get_by(array('product_id' => $product_id), true);
                $inventory_id = $inventory->inventory_id;
                //Create a Variable from a string
                $field = '$'.'inventory->product_quantity_'.$country;
                eval("\$field = \"$field\";");

                $inventory_qty['product_quantity_'.$country] = $item['qty'] + $field;
    ...
    public function pdf_invoice($id)
    {
        ...
        $html = $this->load->view('admin/purchase/pdf_invoice', $data, true);
        $filename =  'PUR-'.$data['purchase']->purchase_order_number;
