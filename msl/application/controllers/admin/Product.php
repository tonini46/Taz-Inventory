...
class Product extends Admin_Controller
{
    ...
    $this->global_model->log('added a category');
    ...
    public function delete_category($id)
    {
        ...
        $this->global_model->log('deleted a category');
    ...
    public function save_subcategory($id = null)
    {
        ...
        $this->global_model->log('added a subcategory');
        //redirect users to view page
        set_message($type, $message);
    ...
    public function delete_subcategory($id)
    {
        ...
        $this->global_model->log('deleted a subcategory');
        //redirect users to view page
        set_message($type, $message);
        redirect('admin/product/subcategory');
    }
    ...
    public function save_product($id = null)
    {
        ...
        $countries = $this->global_model->get_countries();
         foreach ($countries as $value) {
           $inventory['product_quantity_'.$value->name] = $this->input->post('product_quantity_'.$value->name, true);
         }

        $inventory['product_id'] = $product_id;

        $this->global_model->save($inventory, $inventory_id);
        $this->global_model->log('added a product:'.$product_info['product_name']);
        $type = 'success';
    ...
    public function manage_product()
    {
        ...
        $data['country'] = country($this->session->userdata('user_country'));
    ...
    public function damage_product()
    {
        ...
        if($this->session->userdata('user_type')==1){
                ...
                $data['damage_product'] = $this->global_model->get_by(array('country_id' => $this->session->userdata('user_country')), false);
            }
    ...
    public function save_damage_product(){
        ...
        $country = null;
        $this->global_model->log('added a damaged product');
        if($this->session->userdata('user_type')==1){
        ...
        //Create a Variable from a string
                $field = '$'.'inventory->product_quantity_'.country($country);
                eval("\$field = \"$field\";");
        ...
        }else
        {
            ...
            if($data['decrease']==1)
            {
                ...
                $sdata['product_quantity_'.country($country)] = $field - $data['qty'];
                $this->tbl_inventory('inventory_id');
    ...
    public function delete_product($id){
        $this->global_model->log('deleted a product');
        if(is_array($id))
        {
            foreach($id as $v_id)
            {
    ...
    public function add_to_print(){
        ...
        $qty = array_values(array_filter($qty));
    ...
    public function barcode_pdf(){
        ...
        $file_name = $this->pdf->pdf_create($html, $view_file, 'download');
        echo $file_name;
    }
