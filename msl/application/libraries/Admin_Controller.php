...
class Admin_Controller extends MY_Controller
{
    public function __construct()
    {
        ...
        foreach($product_inventory as $v_inventory){
                                        ...
                                        $total_quantity = null;
                                        $i=0;
                                        //$notify_products = array();
                                        //Admin
                                        if($this->session->userdata('user_type') == 1){
                                            foreach ($this->global_model->get_countries() as $key) {
                                            $i++;
                                            //Create a Variable from a string
                                            $field = '$'.'v_inventory->product_quantity_'.$key->name;
                                            eval("\$field = \"$field\";");
                                            $total_quantity += $field;
                                            }

                                            if($v_inventory->notify_quantity*$i >= $total_quantity)
                $notify_products[] = $this->global_model->get_product_bellow_qty($v_inventory->product_id ,$total_quantity,$i);
                                        }
                                            //user
                                        else if($this->session->userdata('user_type') == 0){
                                            //Create a Variable from a string
                                            $field = '$'.'v_inventory->product_quantity_'.country($this->session->userdata('user_country'));
                                            eval("\$field = \"$field\";");
                                            if($v_inventory->notify_quantity >= $field)
                $notify_products[] = $this->global_model->get_product_bellow_qty($v_inventory->product_id ,$field,null);
                                        }
if(!empty($notify_products))
                $notify_product['inventory'] = array_filter($notify_products);
else    
$notify_product['inventory'] = null;            
                $notify_product['countries'] = $this->global_model->get_countries();
                $notify_product['user_type'] = $this->session->userdata('user_type');
            if($this->session->userdata('user_type')==0){
                $notify_product['country'] = country($this->session->userdata('user_country'));
                }
        }
//echo json_encode($notify_products);exit;
        ...
        else if($this->session->userdata('user_type') == 0){//Users
        $pending_order = $this->global_model->get_by(array("order_status"=> 0,"country_id"=>$this->session->userdata('user_country')), false);
        }
    ...
    /* product Currency New table */

    public function tbl_currency_converter_new($order_by){
        $this->global_model->_table_name = 'tbl_currency_converter_new';
        $this->global_model->_order_by = $order_by;
    ...
    public function tbl_country($order_by){
        $this->global_model->_table_name = 'tbl_country';
        $this->global_model->_order_by = $order_by;
        $this->global_model->_primary_key = 'country_id';
    }

     /* expense table */

    public function tbl_expense($order_by){
