...
class Menu
{
    public function dynamicMenu()
    {
        ...
        if ($user_type != 1) {
            ...
            $user_menu = $db->query("SELECT tbl_user_role.menu_id, tbl_user_role.employee_login_id, tbl_menu.*
                                        FROM tbl_user_role
                                        INNER JOIN tbl_menu
           ...
           $user_menu =  $db->query('SELECT menu_id, label, link, icon, parent FROM tbl_menu WHERE menu_id!=31 ORDER BY sort');
        }
