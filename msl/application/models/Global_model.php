...
class Global_Model extends MY_Model
{
    ...
    public function get_convert($currency_code=null, $price=null){
        $default_currency = $this->global_model->get_default_currency();
        if($currency_code != $default_currency){
               $this->load->model('crate_model');
               $currency_info = $this->crate_model->get_c_rate_rule_info($currency_code);
               $rate = $currency_info->rate_form_usd;
               return $rate*$price;
           }
        else{
            return $price;
