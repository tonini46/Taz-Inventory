...
class Order_Model extends MY_Model
{
    ...
    public function get_all_product_info()
    {
        ...
        $product_quantity_country = null;
        $countries = $ci->global_model->get_countries();
        foreach ($countries as $value) {
            $product_quantity_country.= 'tbl_inventory.product_quantity_'.$value->name.','; 
        }
        $this->db->select('tbl_product.*', false);
        $this->db->select($product_quantity_country.'tbl_inventory.notify_quantity ', false);
    ...
    public function get_customer_details($customer_code)
    {
        ...
        $this->db->where($where);
       // if($user_type == 0){
       //     $this->db->where('tbl_customer.country_id', $this->session->userdata('user_country'));
       //     }
        $query_result = $this->db->get();
        $result = $query_result->row();
        return $result;
    ...
    public function get_all_order()
    {
        ...
        //filter by country and Roles
        if($this->session->userdata('user_type') == 0){
        $this->db->where('country_id',$this->session->userdata('user_country'));
        }
    ...
    public function get_all_invoice()
    {
        ...
        public function get_one_invoice($id)
    {
        $this->db->select('tbl_invoice.*, tbl_order.order_id', false);
        $this->db->from('tbl_invoice');
        $this->db->join('tbl_order', 'tbl_order.order_id  =  tbl_invoice.order_id ', 'left');
